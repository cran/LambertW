...
#' scale \eqn{s}, and degrees of freedom \eqn{\nu}. If parameters are not specified, 
#' the MLE given the data will be used (see \code{\link[MASS]{fitdistr}}).
#' 
#' For estimated parameters of the t-distribution the p-values are incorrect and
#' should be adjusted. See \code{\link[stats]{ks.test}} and the references therein 
#' (Durbin (1973)).
#' 
...
#' in which case it will be estimated from the data \code{y}.
#' @return 
#' A list of class \code{"htest"} containing:
#' \item{statistic}{the value of the Kolomogorv-Smirnov statistic.}
#' \item{p.value }{the p-value for the test.} 
#' \item{alternative }{a character string describing the alternative hypothesis.} 
#' \item{method}{the character string "One-sample Kolmogorov-Smirnov test student-t" 
#' plus rounded parameter values.} 
...
ks.test.t <- function(x, param = NULL) {
  ...
  test.result <- suppressWarnings(ks.test(standardized.data, "pt", df = param['df']))
