...
#' 
#' 
#' @param method string; should a fast \code{"Taylor"} (default) approximation be used 
...
get_initial_theta <- function(y, distname, type = c("h", "hh", "s"),
  ...
  if (method == "Taylor") {
    tau.init <- get_initial_tau(y, type = type, location.family = location.family)
  } else if (method == "IGMM") {
    # For a location family use skewness 0 and kurtosis 3 as the target.
    # Use exponential distribution with skewness 2 and kurtosis 9 if it's not a location family.
    tau.init <- IGMM(y, type = type, skewness.x = ifelse(location.family, 0, 2),
