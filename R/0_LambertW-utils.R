...
#' generation for a Lambert W \eqn{\times} \eqn{F_X(x \mid \boldsymbol
#' \beta)} random variable with parameter \eqn{\theta = (\alpha, \boldsymbol \beta,
#' \gamma, \delta)}.
#' 
#' Following the usual R notation and functionality (e.g., \code{rnorm},
#' \code{dnorm}, ...) the Lambert W\eqn{ \times} F utility functions work as expected: 
#' \code{dLambertW} evaluates the pdf at \code{y}, 
...
#' 
#' @details
#'  All functions here have an optional \code{input.u} argument where users
#' can supply their own version corresponding to zero-mean, 
#' unit variance input \eqn{U}.  This function usually depends on 
#' the input parameter \eqn{\boldsymbol \beta}; e.g., users can pass their own 
#' density function \code{dmydist <- function(u, beta) {...})} as
#' \code{dLambertW(..., input.u = dmydist}.  \code{dLambertW} will then use this
#' function to evaluate the pdf of you Lambert W x 'mydist' distribution.
#' 
#' \strong{Important:} Make sure that all \code{input.u} in 
#' \code{dLambertW}, \code{pLambertW}, ... are supplied correctly -- there are no
#' tests or checks for user-defined functions.
#' 
#' 
#' See the references for the analytic expressions of the pdf and cdf.
#' Quantiles are computed numerically for \code{type = "s"} case (no
#' analytic expression); for \code{"h"} or \code{"hh"} quantiles can be
#' computed analytically given the input quantile function and \eqn{\theta}.
#' 
#' @inheritParams common-arguments
#' @param y,q vector of quantiles.
#' @param p vector of probability levels
#' @param n number of observations
#' @param theta pass parameters \code{alpha}, \code{beta}, \code{gamma}, \code{delta} as a list. 
#' If \code{theta} is supplied, the arguments \code{alpha}, \code{beta}, \code{gamma}, \code{delta}
#' are overwritten by corresponding (default) values in \code{theta}.
#' @param gamma optional; skewness parameter; default: \code{0}.
#' @param delta optional; heavy-tail parameter; default: \code{0}.
#' @param alpha optional; heavy tail exponent; default: \code{1}.
#' @param log logical; if \code{TRUE}, probabilities p are given as log(p).
#' @param return.x logical; if \code{TRUE} not only the simulated Lambert W\eqn{ \times} F sample
#' \code{y}, but also the corresponding simulated input \code{x} will be returned. 
#' Default \code{FALSE}. \strong{Note:} if \code{TRUE} then \code{rLambertW} does not
#' return a vector of length \code{n}, but a list of two vectors (each of length \code{n}).
...
#' simulated values or a function defining the pdf/cdf/quanitle function of U);
#' default: \code{NULL}. If not \code{NULL}, \code{tau} must be specified as well.
#' @param tau optional; if \code{input.u = TRUE}, then \code{tau} must be specified.  
#' Note that \eqn{\beta} is still taken from \code{theta}, but
#' \code{"mu_x"}, \code{"sigma_x"}, and the other parameters (\eqn{\alpha, \gamma, \delta})
#' are all taken from \code{tau}.  Note that this is usually only used by
#' the \code{\link{create_LambertW_output}} function; users usually don't
...
#' and \code{distname} (currrently, this only works for \code{distname = "normal"}):
#' \item{mean}{mean,} 
